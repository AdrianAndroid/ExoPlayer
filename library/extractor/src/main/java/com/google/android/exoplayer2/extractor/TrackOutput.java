...
public interface TrackOutput {
    ...
    /**
     * Sample encryption data. 样本加密数据
     *
     * <p>The format for encryption information is: 加密信息的格式为：
     *
     * <ul>
    ...
    /**
     * Called when the {@link Format} of the track has been extracted from the stream. 当从流提取轨道的格式时调用。
     *
    ...
    /**
     * Called to write sample data to the output. 调用以将示例数据写入输出。
     *
     * @param input           A {@link DataReader} from which to read the sample data. 从中读取示例数据的DataReader.
     * @param length          The maximum length to read from the input.  从输入读取的最大长度。
     * @param allowEndOfInput True if encountering the end of the input having read no data is
     *                        allowed, and should result in {@link C#RESULT_END_OF_INPUT} being returned. False if it
     *                        should be considered an error, causing an {@link EOFException} to be thrown.
     *                        如果遇到没有读取任何数据的输入的结尾，则为True，并且应导致返回RESULT_END_OF_INPUT。如果应该在错误时考虑它，则为False，导致抛出异常
     * @param sampleDataPart  The part of the sample data to which this call corresponds. 此调用对应的样本数据部分。
     * @return The number of bytes appended.  附加的字节数.
     * @throws IOException If an error occurred reading from the input.  如果从输入读取时发生错误。
     */
    int sampleData(DataReader input, int length, boolean allowEndOfInput, @SampleDataPart int sampleDataPart) throws IOException;

    /**
     * Called to write sample data to the output.å
     *
