...
public final class FlacExtractor implements Extractor {
    ...
    public FlacExtractor(int flags) {
        streamMarkerAndInfoBlock = new byte[FlacConstants.STREAM_MARKER_SIZE + FlacConstants.STREAM_INFO_BLOCK_SIZE];
    ...
    private void readMetadataBlocks(ExtractorInput input) throws IOException {
        ...
        castNonNull(trackOutput).format(flacStreamMetadata.getFormat(streamMarkerAndInfoBlock, id3Metadata));
    ...
    private @ReadResult
    int readFrames(ExtractorInput input, PositionHolder seekPosition)
        throws IOException {
        ...
        if (buffer.bytesLeft() < FlacConstants.MAX_FRAME_HEADER_SIZE) {
            ...
            System.arraycopy(buffer.getData(), buffer.getPosition(), buffer.getData(), /* destPos= */ 0, bytesLeft);
    ...
    private long findFrame(ParsableByteArray data, boolean foundEndOfInput) {
        ...
        if (foundEndOfInput) {
            ...
            while (frameOffset <= data.limit() - minFrameSize) {
                ...
                boolean frameFound;
                try {
                    frameFound = FlacFrameReader.checkAndReadFrameHeader(
                        data, flacStreamMetadata, frameStartMarker, sampleNumberHolder);
    ...
    private void outputSampleMetadata() {
        long timeUs = currentFrameFirstSampleNumber * C.MICROS_PER_SECOND / castNonNull(flacStreamMetadata).sampleRate;
