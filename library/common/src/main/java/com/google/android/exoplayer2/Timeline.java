...
public abstract class Timeline implements Bundleable {
    ...
    public static final class Window implements Bundleable {
        ...
        public int hashCode() {
            ...
            result = 31 * result + (int) (elapsedRealtimeEpochOffsetMs ^ (elapsedRealtimeEpochOffsetMs >>> 32));
        ...
        private final Bundle toBundle(boolean excludeMediaItem) {
            ...
            bundle.putBundle(keyForField(FIELD_MEDIA_ITEM), excludeMediaItem ? MediaItem.EMPTY.toBundle() : mediaItem.toBundle());
            ...
            bundle.putLong(keyForField(FIELD_ELAPSED_REALTIME_EPOCH_OFFSET_MS), elapsedRealtimeEpochOffsetMs);
        ...
        private static Window fromBundle(Bundle bundle) {
            ...
            long presentationStartTimeMs = bundle.getLong(keyForField(FIELD_PRESENTATION_START_TIME_MS), /* defaultValue= */ C.TIME_UNSET);
            long windowStartTimeMs = bundle.getLong(keyForField(FIELD_WINDOW_START_TIME_MS), /* defaultValue= */ C.TIME_UNSET);
            long elapsedRealtimeEpochOffsetMs = bundle.getLong(keyForField(FIELD_ELAPSED_REALTIME_EPOCH_OFFSET_MS),/* defaultValue= */ C.TIME_UNSET);
            boolean isSeekable = bundle.getBoolean(keyForField(FIELD_IS_SEEKABLE), /* defaultValue= */ false);
            boolean isDynamic = bundle.getBoolean(keyForField(FIELD_IS_DYNAMIC), /* defaultValue= */ false);
            @Nullable
            ...
            boolean isPlaceHolder = bundle.getBoolean(keyForField(FIELD_IS_PLACEHOLDER), /* defaultValue= */ false);
            long defaultPositionUs = bundle.getLong(keyForField(FIELD_DEFAULT_POSITION_US), /* defaultValue= */ 0);
            long durationUs = bundle.getLong(keyForField(FIELD_DURATION_US), /* defaultValue= */ C.TIME_UNSET);
            int firstPeriodIndex = bundle.getInt(keyForField(FIELD_FIRST_PERIOD_INDEX), /* defaultValue= */ 0);
            int lastPeriodIndex = bundle.getInt(keyForField(FIELD_LAST_PERIOD_INDEX), /* defaultValue= */ 0);
            long positionInFirstPeriodUs = bundle.getLong(keyForField(FIELD_POSITION_IN_FIRST_PERIOD_US), /* defaultValue= */ 0);
    ...
    public static final class Period implements Bundleable {
        ...
        private static Period fromBundle(Bundle bundle) {
            ...
            long durationUs = bundle.getLong(keyForField(FIELD_DURATION_US), /* defaultValue= */ C.TIME_UNSET);
            long positionInWindowUs = bundle.getLong(keyForField(FIELD_POSITION_IN_WINDOW_US), /* defaultValue= */ 0);
            ...
            AdPlaybackState adPlaybackState = adPlaybackStateBundle != null
    ...
    public int getNextWindowIndex(int windowIndex, @Player.RepeatMode int repeatMode, boolean shuffleModeEnabled) {
        switch (repeatMode) {
            case Player.REPEAT_MODE_OFF:
                return windowIndex == getLastWindowIndex(shuffleModeEnabled) ? C.INDEX_UNSET : windowIndex + 1;
    ...
    public final Bundle toBundle(boolean excludeMediaItems) {
        ...
        for (int i = 0; i < windowCount; i++) {
            windowBundles.add(getWindow(i, window, /* defaultPositionProjectionUs= */ 0).toBundle(excludeMediaItems));
        }
        ...
        for (int i = 1; i < windowCount; i++) {
            shuffledWindowIndices[i] = getNextWindowIndex(shuffledWindowIndices[i - 1], Player.REPEAT_MODE_OFF, /* shuffleModeEnabled= */ true);
        }
    ...
    private static Timeline fromBundle(Bundle bundle) {
        ImmutableList<Window> windows = fromBundleListRetriever(Window.CREATOR, BundleUtil.getBinder(bundle, keyForField(FIELD_WINDOWS)));
        ImmutableList<Period> periods = fromBundleListRetriever(Period.CREATOR, BundleUtil.getBinder(bundle, keyForField(FIELD_PERIODS)));
        @Nullable
        int[] shuffledWindowIndices = bundle.getIntArray(keyForField(FIELD_SHUFFLED_WINDOW_INDICES));
        return new RemotableTimeline(
                windows,
                periods,
                shuffledWindowIndices == null ? generateUnshuffledIndices(windows.size()) : shuffledWindowIndices);
    }

    private static <T extends Bundleable> ImmutableList<T> fromBundleListRetriever(Creator<T> creator, @Nullable IBinder binder) {
        if (binder == null) {
    ...
    public static final class RemotableTimeline extends Timeline {
        ...
        public int getNextWindowIndex(int windowIndex, @Player.RepeatMode int repeatMode, boolean shuffleModeEnabled) {
            ...
            if (windowIndex == getLastWindowIndex(shuffleModeEnabled)) {
                return repeatMode == Player.REPEAT_MODE_ALL ? getFirstWindowIndex(shuffleModeEnabled) : C.INDEX_UNSET;
            }
            return shuffleModeEnabled ? shuffledWindowIndices[windowIndicesInShuffled[windowIndex] + 1] : windowIndex + 1;
        }
        ...
        public int getPreviousWindowIndex(int windowIndex, @Player.RepeatMode int repeatMode, boolean shuffleModeEnabled) {
            ...
            if (windowIndex == getFirstWindowIndex(shuffleModeEnabled)) {
                return repeatMode == Player.REPEAT_MODE_ALL ? getLastWindowIndex(shuffleModeEnabled) : C.INDEX_UNSET;
            }
            return shuffleModeEnabled ? shuffledWindowIndices[windowIndicesInShuffled[windowIndex] - 1] : windowIndex - 1;
        }
        ...
        public int getLastWindowIndex(boolean shuffleModeEnabled) {
            ...
            return shuffleModeEnabled ? shuffledWindowIndices[getWindowCount() - 1] : getWindowCount() - 1;
        }
