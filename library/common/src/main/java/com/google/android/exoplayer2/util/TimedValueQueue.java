 ...
 * 一个实用程序类，用于保存带有时间戳的值队列。这个类是线程安全的。
 */
public final class TimedValueQueue<V> {

    private void log(String msg) {
        Log.i("TimedValueQueue", msg);
    }
    ...
    public synchronized void add(long timestamp, V value) {
        log("add(timestamp=" + timestamp + " ,value=" + value);
    ...
    public synchronized V pollFloor(long timestamp) {
        log("pollFloor(timestamp=" + timestamp);
    ...
    private V poll(long timestamp, boolean onlyOlder) {
        log("poll(timestamp=" + timestamp + " ,onlyOlder=" + onlyOlder);
    ...
    private void clearBufferOnTimeDiscontinuity(long timestamp) {
        log("clearBufferOnTimeDiscontinuity(timestamp=" + timestamp);
        if (size > 0) {
    ...
    private void addUnchecked(long timestamp, V value) {
        log("addUnchecked(timestamp=" + timestamp + " ,value=" + value);
    ...
    private static <V> @NullableType V[] newArray(int length) {
        Log.i("TimedValueQueue", "newArray(length=" + length);
