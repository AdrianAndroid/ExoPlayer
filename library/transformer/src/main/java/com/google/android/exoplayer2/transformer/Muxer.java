 ...
 *
 * <p>查询{@link #supportsSampleMimeType(String) sample MIME types are supported}和
 * {@link #addTrack(Format) add all track}，然后{@link #writeSampleData(int, ByteBuffer, boolean,
 * long) 将样本数据}写入多路复用样本。 一旦写入任何样本数据，就无法添加曲目。
 * 写入所有示例数据后，{@link #release() 释放} 实例以完成写入输出并将任何资源返回给系统。
 */
