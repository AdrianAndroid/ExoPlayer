...
/**
 * A transformer to transform media inputs. 转换媒体输入的转换器
 *
 * <p>The same Transformer instance can be used to transform multiple inputs (sequentially, not
 * concurrently). 同一个 Transformer 实例可用于转换多个输入（按顺序，而不是同时）。
 *
 ...
 * 必须从单个应用程序线程访问转换器实例。
 * 必须在其上访问Transformerinstance 的线程可以通过在创建转换器时传递 Looper 来显式指定.
 * 如果未指定 Looper，则使用创建 Transfomer.Builder 的线程的 Looper，或者如果该线程没有 Looper，则使用应用程序主线程的 Looper。
 * 在所有情况下，都可以使用 getApplicationLooper() 查询必须从中访问转换器的线程的 Looper
 */
@RequiresApi(18)
