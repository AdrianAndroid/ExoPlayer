...
/* package */ class SampleDataQueue {
    ...
    /**
     * Reads data from the rolling buffer to populate a decoder input buffer. 从滚动缓冲区读取数据以填充解码器输入缓冲区
     *
    ...
    /**
     * Reads data from {@code allocationNode} and its following nodes. 从AllocationNode及其后续节点读取数据
     *
     * @param allocationNode   The first {@link AllocationNode} containing data yet to be read.
     * @param absolutePosition The absolute position from which data should be read.
