...
public interface AudioSink {
     ...
     * #CURRENT_POSITION_NOT_SET} if it is not yet available. 返回流中从零开始的播放位置，以微秒为单位，
     * 如果尚不可用，则返回CURRENT_POSITION_NOT_SET
     *
     ...
     * 尝试处理来自ByteBuffer的数据，从其当前位置开始并在其限制（不包括）结束。ByteBuffer的位置提前了处理的字节数。
     * 如果presentationTimeUs与自上次重置依赖处理的最后一个缓冲区不连续，则将调用onPositionDiscontinuity.
     *
     ...
     * 返回数据是否被完整处理。如果数据没有被完全处理，那么必须像后续调用提供相同的ByteBuffer知道它被完全消耗，除非中间
     * 调用flush()（或configure导致接收器被刷新）
     *
     ...
     * 返回是否已调用 playToEndOfStream 并已处理所有缓冲区
     */
    boolean isEnded();
     ...
     * are not supported. 尝试设置播放参数。如果不支持，音频接收器可能会覆盖这些参数。
     *
     ...
     * 设置音频播放的属性。如果属性已更改，并且如果接收器未配置为与隧道一起使用，则将其重置并清除音频查看ID。
     ...
     * 如果将接收器配置为与隧道一起使用，则音频属性将被忽略。接收器不会被重置，并且音频播放ID不会被清除。如果接收器稍后重新
     * 配置为非隧道模式，则将使用传递的属性。
     *
    ...
    /**
     * Sets the audio session id. 设置音频会话 ID。
     */
    void setAudioSessionId(int audioSessionId);

    /**
     * Sets the auxiliary effect. 设置辅助效果
     */
     ...
     * requires platform API version 21 onwards. 如果可能，启用隧道。接收器隧道以前被禁用。仅当接收器基于平台
     * AudioTrack并且需要平台API版本21时才能启用隧道.
     *
     ...
     * session id is cleared. 禁用隧道。如果之前启用了隧道，则将重置接收器并清除任何音频会话ID。
     */
