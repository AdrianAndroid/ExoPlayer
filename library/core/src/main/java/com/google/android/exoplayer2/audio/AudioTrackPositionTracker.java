...
/* package */ final class AudioTrackPositionTracker {
     ...
     * track's position, until the next call to {@link #reset()}. 将AudioTrack设置为环绕（封装？）。此实例的后续方法调用与此轨道的位置相关，直到下一次调用reset()
     *
     * @param audioTrack         The audio track to wrap.  要环绕的音轨
     * @param isPassthrough      Whether passthrough mode is being used. 是否正在使用直通模式
     * @param outputEncoding     The encoding of the audio track. 音轨的编码
     * @param outputPcmFrameSize For PCM output encodings, the frame size. The value is ignored otherwise. 对于PCM输出编码，帧大小。否则该值被忽略。
     * @param bufferSize         The audio track buffer size in bytes. 音轨缓冲区大小(以字节为单位).
     */
    public void setAudioTrack(AudioTrack audioTrack, boolean isPassthrough, @C.Encoding int outputEncoding, int outputPcmFrameSize, int bufferSize) {
