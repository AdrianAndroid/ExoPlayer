 ...
 * 定义并提供要由ExoPlayer播放的媒体。MediaSource由两个主要职责。
 *
 * <ul>
 ...
 *       TransferListener)}.
 *       为播放器提供定义其媒体结构的时间轴。并在媒体结构发生变化时提供新的时间表。
 *       MediaSource 通过在传递给 PrepareSource#Listener 的 MediaSourceCallers 上
 *       调用 MediaSourceCaller#onSourceInfoRefreshed 来提供这些时间线
 *   <li>To provide {@link MediaPeriod} instances for the periods in its timeline. MediaPeriods are
 *       obtained by calling {@link #createPeriod(MediaPeriodId, Allocator, long)}, and provide a
 *       way for the player to load and read the media.
 *       为其时间线中的期间提供 MediaPeriod 实例。 MediaPeriods 是通过调用 CreatePeriod 获得的，并为播放器提供了
 *       一种加载和读取 meida 的方法。
 * </ul>
 * <p>
 ...
 * 所有方法都在播放器的内部播放线程上调用，如 ExoPlayer javadoc 中所述。 不应同时从 ExoPlayer 实例直接调用它们。
 */
