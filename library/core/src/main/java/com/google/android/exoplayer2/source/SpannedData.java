...
/**
 * Stores value objects associated with spans of integer keys. 存储与整数键跨度关联的值对象。
 *
 * <p>This implementation is optimised for consecutive {@link #get(int)} calls with keys that are
 * close to each other in value. 此实现针对具有彼此接近的键值的连续GET调用进行了优化。
 *
 * <p>Spans are defined by their own {@code startKey} (inclusive) and the {@code startKey} of the
 * next span (exclusive). The last span is open-ended. Span由自己的StartKey（含）和下一个span的StartKey（不含）定义。 最后一个跨度是开放式的。
 *
 * @param <V> The type of values stored in this collection. 此集合中存储的值的类型。
 */
