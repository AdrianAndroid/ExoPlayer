 ...
 * the appropriate rate to detect when the timestamp starts to advance. 如果平台支持，则轮询AudioTrack时间戳，并在适当的时候进行轮询。检测时间戳何时开始提前的速率。
 * <p>
 ...
 * 当音轨未暂停时，定期调用 {@link #maybePollTimestamp(long)} 以检查时间戳更新。 如果返回 {@code true}，则调用 {@link #getTimestampPositionFrames()} 和 {@link #getTimestampSystemTimeUs()} 访问更新的时间戳，然后调用 {@link #acceptTimestamp()} 或 {@link #rejectTimestamp()} 来接受或拒绝它。
 * <p>
 ...
 * <p>
 * 如果 {@link #hasTimestamp()} 返回 {@code true}，则调用 {@link #getTimestampSystemTimeUs()} 获取最新时间戳被采样的系统时间，并调用 {@link #getTimestampPositionFrames()} 获取其位置 在帧中。 如果 {@link #hasAdvancingTimestamp()} 返回 {@code true}，则调用者应假定时间戳自采样以来一直在实时增加。 否则，它可能是静止的。
 *
    ...
    private static final class AudioTimestampV19 {
        ...
        public boolean maybeUpdateTimestamp() {
            ...
            if (updated) {
                ...
                lastTimestampPositionFrames = rawPositionFrames + (rawTimestampFramePositionWrapCount << 32);
            }
            return updated;
        }
