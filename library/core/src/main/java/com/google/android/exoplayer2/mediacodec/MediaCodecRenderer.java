...
public abstract class MediaCodecRenderer extends BaseRenderer {
    ...
    public static class DecoderInitializationException extends Exception {
        ...
        public DecoderInitializationException(Format format, @Nullable Throwable cause, boolean secureDecoderRequired,
                                              int errorCode) {
            this(
     ...
     */
    protected abstract List<MediaCodecInfo> getDecoderInfos(MediaCodecSelector mediaCodecSelector, Format format, boolean requiresSecureDecoder) throws DecoderQueryException;
    ...
    protected final void maybeInitCodecOrBypass() throws ExoPlaybackException {
        //log(getClass(), "maybeInitCodecOrBypass");
        ...
        log(getClass(), "maybeInitCodecOrBypass" + " , sourceDrmSession=" + sourceDrmSession + " , inputFormat=" + inputFormat);
        if (sourceDrmSession == null && shouldUseBypass(inputFormat)) {
            initBypass(inputFormat);
            return;
        }
        setCodecDrmSession(sourceDrmSession);
        log(getClass(), "maybeInitCodecOrBypass" + " , sourceDrmSession=" + sourceDrmSession);
        ...
        if (codecDrmSession != null) {
            if (mediaCrypto == null) {
                ...
                if (sessionMediaCrypto == null) {
                    ...
                    mediaCryptoRequiresSecureDecoder = !sessionMediaCrypto.forceAllowInsecureDecoderComponents && mediaCrypto.requiresSecureDecoderComponent(mimeType);
                }
            }
    ...
    protected void onPositionReset(long positionUs, boolean joining) throws ExoPlaybackException {
        ...
        if (pendingOutputStreamOffsetCount != 0) {
            ...
            outputStreamStartPositionUs = pendingOutputStreamStartPositionsUs[pendingOutputStreamOffsetCount - 1];
    ...
    public void render(long positionUs, long elapsedRealtimeUs) throws ExoPlaybackException {
        super.render(positionUs, elapsedRealtimeUs); // flannery加,打印log用
        if (pendingOutputEndOfStream) {
            log("pendingOutputEndOfStream -> " + pendingOutputEndOfStream);
            pendingOutputEndOfStream = false;
            processEndOfStream();
        }
        if (pendingPlaybackException != null) {
            log("pendingPlaybackException -> " + pendingPlaybackException);
        ...
        try {
            if (outputStreamEnded) { //是否到最后
                log("outputStreamEnded == true");
                renderToEndOfStream();
                return;
            }
            if (inputFormat == null && !readSourceOmittingSampleData(FLAG_REQUIRE_FORMAT)) { // 没有format
                // We still don't have a format and can't make progress without one.
                log("inputFormat == null && !readSourceOmittingSampleData(FLAG_REQUIRE_FORMAT)");
                return;
            }
            // We have a format.
    ...
    protected void resetCodecStateForFlush() {
        ...
        codecReconfigurationState = codecReconfigured ? RECONFIGURATION_STATE_WRITE_PENDING : RECONFIGURATION_STATE_NONE;
    }
    ...
    private List<MediaCodecInfo> getAvailableCodecInfos(boolean mediaCryptoRequiresSecureDecoder) throws DecoderQueryException {
        List<MediaCodecInfo> codecInfos = getDecoderInfos(mediaCodecSelector, inputFormat, mediaCryptoRequiresSecureDecoder);
    ...
    /**
     * Configures rendering where no codec is used. Called instead of  when no codec
     * is used to render. 在不使用编解码器的情况下配置渲染。当没有使用编解码器进行渲染时调用而不是configureCodec().
     */
    ...
    private boolean shouldContinueRendering(long renderStartTimeMs) {
        return renderTimeLimitMs == C.TIME_UNSET || SystemClock.elapsedRealtime() - renderStartTimeMs < renderTimeLimitMs;
    }
        ...
        if (buffer.isEndOfStream()) {
            ...
            try {
                if (codecNeedsEosPropagation) {
                    ...
                    codecReceivedEos = true;
                    codec.queueInputBuffer(inputIndex,/* offset= */ 0,/* size= */ 0,/* presentationTimeUs= */ 0, MediaCodec.BUFFER_FLAG_END_OF_STREAM);
                    resetInputBuffer();
                }
    ...
    private boolean drainOutputBuffer(long positionUs, long elapsedRealtimeUs) throws ExoPlaybackException {
        ...
        if (codecNeedsEosOutputExceptionWorkaround && codecReceivedEos) {
            try {
                processedOutputBuffer = processOutputBuffer(positionUs, elapsedRealtimeUs, codec, outputBuffer, outputIndex, outputBufferInfo.flags,/* sampleCount= */ 1, outputBufferInfo.presentationTimeUs, isDecodeOnlyOutputBuffer, isLastOutputBuffer, outputFormat);
            } catch (IllegalStateException e) {
                processEndOfStream();
                if (outputStreamEnded) {
                    // Release the codec, as it's in an error state.
    ...
    private boolean drmNeedsCodecReinitialization(
        ...
        // output path.
        return !codecInfo.secure && maybeRequiresSecureDecoder(newMediaCrypto, newFormat);
    }
     ...
     */
    private @AdaptationWorkaroundMode
    int codecAdaptationWorkaroundMode(String name) {
        if (Util.SDK_INT <= 25
