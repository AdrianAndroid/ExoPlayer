...
public class DefaultDashChunkSource implements DashChunkSource {
    ...
    public boolean onChunkLoadError(
        ...
        RepresentationHolder representationHolder = representationHolders[trackIndex];
        @Nullable
        BaseUrl newBaseUrl = baseUrlExclusionList.selectBaseUrl(representationHolder.representation.baseUrls);
        ...
        if (fallbackSelection.type == LoadErrorHandlingPolicy.FALLBACK_TYPE_TRACK) {
            cancelLoad = trackSelection
                .blacklist(trackSelection.indexOf(chunk.trackFormat), fallbackSelection.exclusionDurationMs);
    ...
    private LoadErrorHandlingPolicy.FallbackOptions createFallbackOptions(
        ...
        return new LoadErrorHandlingPolicy.FallbackOptions(
            ...
            priorityCount - baseUrlExclusionList.getPriorityCountAfterExclusion(baseUrls),
            numberOfTracks,
            numberOfExcludedTracks);
    }
    ...
    private long getNowPeriodTimeUs(long nowUnixTimeUs) {
        return manifest.availabilityStartTimeMs == C.TIME_UNSET
            ? C.TIME_UNSET
            : nowUnixTimeUs - C.msToUs(manifest.availabilityStartTimeMs + manifest.getPeriod(periodIndex).startMs);
    }
    ...
    protected Chunk newInitializationChunk(
        ...
        DataSpec dataSpec = DashUtil
            .buildDataSpec(representation, representationHolder.selectedBaseUrl.url, requestUri, /* flags= */ 0);
    ...
    protected Chunk newMediaChunk(
        ...
        if (representationHolder.chunkExtractor == null) {
            ...
            int flags = representationHolder.isSegmentAvailableAtFullNetworkSpeed(firstSegmentNum, nowPeriodTimeUs)
                ? 0 : DataSpec.FLAG_MIGHT_NOT_USE_FULL_NETWORK_SPEED;
            DataSpec dataSpec =
                DashUtil.buildDataSpec(representation, representationHolder.selectedBaseUrl.url, segmentUri, flags);
            ...
            long clippedEndTimeUs =
                periodDurationUs != C.TIME_UNSET && periodDurationUs <= endTimeUs ? periodDurationUs : C.TIME_UNSET;
            int flags =
    ...
    private RepresentationHolder updateSelectedBaseUrl(int trackIndex) {
        RepresentationHolder representationHolder = representationHolders[trackIndex];
        @Nullable
        BaseUrl selectedBaseUrl = baseUrlExclusionList.selectBaseUrl(representationHolder.representation.baseUrls);
    ...
    protected static final class RepresentationSegmentIterator extends BaseMediaChunkIterator {
        ...
        public DataSpec getDataSpec() {
            ...
            int flags = representationHolder.isSegmentAvailableAtFullNetworkSpeed(currentIndex, nowPeriodTimeUs)
                ? 0 : DataSpec.FLAG_MIGHT_NOT_USE_FULL_NETWORK_SPEED;
    ...
    protected static final class RepresentationHolder {
        ...
        @CheckResult
            /* package */ RepresentationHolder copyWithNewRepresentation(long newPeriodDurationUs,
            ...
            if (oldIndexEndTimeUs == newIndexStartTimeUs) {
                ...
                newSegmentNumShift -=
                    newIndex.getSegmentNum(oldIndexStartTimeUs, newPeriodDurationUs) - oldIndexFirstSegmentNum;
            } else {
                // The new index overlaps with (and has a start position contained within) the old index.
                newSegmentNumShift +=
                    oldIndex.getSegmentNum(newIndexStartTimeUs, newPeriodDurationUs) - newIndexFirstSegmentNum;
            }
            return new RepresentationHolder(
                newPeriodDurationUs,
                newRepresentation,
        ...
        public long getFirstAvailableSegmentNum(long nowUnixTimeUs) {
            return segmentIndex.getFirstAvailableSegmentNum(periodDurationUs, nowUnixTimeUs) + segmentNumShift;
        }
        ...
        public long getLastAvailableSegmentNum(long nowUnixTimeUs) {
            return getFirstAvailableSegmentNum(nowUnixTimeUs) +
                segmentIndex.getAvailableSegmentCount(periodDurationUs, nowUnixTimeUs) - 1;
        }
