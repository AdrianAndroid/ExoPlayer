...
public final class DashUtil {
     ...
     */
    public static DataSpec buildDataSpec(Representation representation, String baseUrl, RangedUri requestUri,
                                         int flags) {
        log("static DataSpec buildDataSpec(Representation representation, String baseUrl, RangedUri requestUri");
    ...
    public static DataSpec buildDataSpec(Representation representation, RangedUri requestUri, int flags) {
        log("static DataSpec buildDataSpec(Representation representation, RangedUri requestUri, int flags)");
    ...
    public static DashManifest loadManifest(DataSource dataSource, Uri uri) throws IOException {
        log("static DashManifest loadManifest(DataSource dataSource, Uri uri)");
    ...
    public static Format loadFormatWithDrmInitData(DataSource dataSource, Period period) throws IOException {
        log("static Format loadFormatWithDrmInitData(DataSource dataSource, Period period)");
    ...
    public static Format loadSampleFormat(DataSource dataSource, int trackType, Representation representation,
                                          int baseUrlIndex) throws IOException {
        log("static Format loadSampleFormat(DataSource dataSource, int trackType, Representation representation,");
        ...
        try {
            loadInitializationData(chunkExtractor, dataSource, representation, baseUrlIndex, /* loadIndex= */ false);
        } finally {
    ...
    public static ChunkIndex loadChunkIndex(DataSource dataSource, int trackType, Representation representation,
                                            int baseUrlIndex) throws IOException {
        log("static ChunkIndex loadChunkIndex(DataSource dataSource, int trackType, Representation representation,");
        if (representation.getInitializationUri() == null) {
            return null;
        }
        ChunkExtractor chunkExtractor = newChunkExtractor(trackType, representation.format);
        try {
            loadInitializationData(chunkExtractor, dataSource, representation, baseUrlIndex, /* loadIndex= */ true);
        } finally {
            chunkExtractor.release();
        }
    ...
    private static void loadInitializationData(
        ...
        log("private static void loadInitializationData");
    ...
    public static void loadInitializationData(
        ...
        boolean loadIndex)
        throws IOException {
        log("public static void loadInitializationData");
        loadInitializationData(chunkExtractor, dataSource, representation, /* baseUrlIndex= */ 0, loadIndex);
    }
    ...
    private static void loadInitializationData(
        ...
        throws IOException {
        log("private static void loadInitializationData(");
        DataSpec dataSpec = DashUtil.buildDataSpec(
            representation,
            representation.baseUrls.get(baseUrlIndex).url,
            requestUri,
            /* flags= */ 0);
        InitializationChunk initializationChunk = new InitializationChunk(
            dataSource,
            dataSpec,
    ...
    public static String resolveCacheKey(Representation representation, RangedUri rangedUri) {
        log("static String resolveCacheKey(Representation representation, RangedUri rangedUri)");
    ...
    private static ChunkExtractor newChunkExtractor(int trackType, Format format) {
        log("static ChunkExtractor newChunkExtractor(int trackType, Format format)");
        String mimeType = format.containerMimeType;
        boolean isWebm = mimeType != null
    ...
    private static Representation getFirstRepresentation(Period period, int type) {
        log("static Representation getFirstRepresentation(Period period, int type)");
