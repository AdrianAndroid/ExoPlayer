...
public class IntentUtil {
    ...
    private static void log(String key, String msg) {
        Log.i("IntentUtil", key + " : " + msg);
    }
    ...
    public static void addToIntent(List<MediaItem> mediaItems, Intent intent) {
        ...
        if (mediaItems.size() == 1) {
            ...
            log("setAction", ACTION_VIEW);
            log("setData", mediaItem.playbackProperties.uri.toString());
            if (mediaItem.mediaMetadata.title != null) {
                intent.putExtra(TITLE_EXTRA, mediaItem.mediaMetadata.title);
                log(TITLE_EXTRA, mediaItem.mediaMetadata.title.toString());
            }
            addPlaybackPropertiesToIntent(playbackProperties, intent, /* extrasKeySuffix= */ "");
            addClippingPropertiesToIntent(mediaItem.clippingProperties, intent, /* extrasKeySuffix= */ "");

            // 就一个文件
            // IntentUtil: setAction : com.google.android.exoplayer.demo.action.VIEW
            // IntentUtil: setData : https://storage.googleapis.com/wvmedia/clear/h264/tears/tears.mpd
            // IntentUtil: title : HD (MP4, H264)
            // IntentUtil: mime_type : application/dash+xml
            // IntentUtil: ad_tag_uri :
        } else {
            intent.setAction(ACTION_VIEW_LIST);
            log("setAction", ACTION_VIEW_LIST);
            for (int i = 0; i < mediaItems.size(); i++) {
                MediaItem mediaItem = mediaItems.get(i);
                MediaItem.PlaybackProperties playbackProperties = checkNotNull(mediaItem.playbackProperties);
                intent.putExtra(URI_EXTRA + ("_" + i), playbackProperties.uri.toString());
                log(URI_EXTRA + ("_" + i), playbackProperties.uri.toString());
                addPlaybackPropertiesToIntent(playbackProperties, intent, /* extrasKeySuffix= */ "_" + i);
                addClippingPropertiesToIntent(mediaItem.clippingProperties, intent, /* extrasKeySuffix= */ "_" + i);
                if (mediaItem.mediaMetadata.title != null) {
                    intent.putExtra(TITLE_EXTRA + ("_" + i), mediaItem.mediaMetadata.title);
                    log(TITLE_EXTRA + ("_" + i), mediaItem.mediaMetadata.title + "");
                }
            }

            // 多个列表
            // setAction : com.google.android.exoplayer.demo.action.VIEW_LIST
            // uri_0 : https://storage.googleapis.com/exoplayer-test-media-1/gen-3/screens/dash-vod-single-segment/audio-141.mp4
            // mime_type_0 : null
            // ad_tag_uri_0 :
            // uri_1 : https://storage.googleapis.com/exoplayer-test-media-1/mkv/android-screens-lavf-56.36.100-aac-avc-main-1280x720.mkv
            // mime_type_1 : null
            // ad_tag_uri_1 :
            // uri_2 : https://storage.googleapis.com/exoplayer-test-media-1/gen-3/screens/dash-vod-single-segment/audio-141.mp4
            // mime_type_2 : null
            // ad_tag_uri_2 :
        }
    }
    ...
    private static void addPlaybackPropertiesToIntent(
        ...
        intent.putExtra(MIME_TYPE_EXTRA + extrasKeySuffix, playbackProperties.mimeType)
            .putExtra(AD_TAG_URI_EXTRA + extrasKeySuffix,
                playbackProperties.adsConfiguration != null
                    ? playbackProperties.adsConfiguration.adTagUri.toString()
                    : null);
        log(MIME_TYPE_EXTRA + extrasKeySuffix, playbackProperties.mimeType);
        log(AD_TAG_URI_EXTRA + extrasKeySuffix,
            playbackProperties.adsConfiguration != null
                ? playbackProperties.adsConfiguration.adTagUri.toString()
                : "");
        ...
        if (!playbackProperties.subtitles.isEmpty()) {
            ...
            log(SUBTITLE_URI_EXTRA + extrasKeySuffix, subtitle.uri.toString());
            log(SUBTITLE_MIME_TYPE_EXTRA + extrasKeySuffix, subtitle.mimeType);
            log(SUBTITLE_LANGUAGE_EXTRA + extrasKeySuffix, subtitle.language);
        }
    }
    ...
    private static void addDrmConfigurationToIntent(
        ...
        log(DRM_SCHEME_EXTRA + extrasKeySuffix, drmConfiguration.uuid.toString());
        intent.putExtra(DRM_LICENSE_URI_EXTRA + extrasKeySuffix,
            drmConfiguration.licenseUri != null ? drmConfiguration.licenseUri.toString() : null);
        log(DRM_LICENSE_URI_EXTRA + extrasKeySuffix,
        ...
        log(DRM_MULTI_SESSION_EXTRA + extrasKeySuffix, drmConfiguration.multiSession + "");
        intent.putExtra(DRM_FORCE_DEFAULT_LICENSE_URI_EXTRA + extrasKeySuffix, drmConfiguration.forceDefaultLicenseUri);
        log(DRM_FORCE_DEFAULT_LICENSE_URI_EXTRA + extrasKeySuffix, drmConfiguration.forceDefaultLicenseUri + "");
        ...
        log(DRM_KEY_REQUEST_PROPERTIES_EXTRA + extrasKeySuffix, Arrays.toString(drmKeyRequestProperties));
        ...
        if (!drmSessionForClearTypes.isEmpty()) {
            ...
            log(DRM_SESSION_FOR_CLEAR_CONTENT + extrasKeySuffix, "true");
        }
    }
    ...
    private static void addClippingPropertiesToIntent(
        ...
        if (clippingProperties.startPositionMs != 0) {
            ...
            log(CLIP_START_POSITION_MS_EXTRA + extrasKeySuffix, clippingProperties.startPositionMs + "");
        }
        if (clippingProperties.endPositionMs != C.TIME_END_OF_SOURCE) {
            ...
            log(CLIP_END_POSITION_MS_EXTRA + extrasKeySuffix, clippingProperties.endPositionMs + "");
        }
    }
}
